...
Nosotros hemos aterrizado en este nuevo y valiente mundo de desacoplamiento, Api-Primeras arquitecturas que permiten la Gestión distribuida y la Entrega de los Modelos de Contenido Estructurado. Donde el contenido viaja constantemente entre las instancias de software apiladas de los servidores y clientes: Hay un servidor de contenido estructurado central (SCS), hay una app editorial para adminitstrar el contenido almacenado en ese servidor (CMS) y hay uno o mas aplicaciones web cliente (CWA) publicando ese contenido y satisfaciendo las necesidades de los usuarios en múltiples maneras.

Para hacer esto el contenido en nuestra aplicación heredada de Drupal debe ser auditada, inventoriada, analizada y conservada para determinar el nuevo y mejorado modelo  de contenido del propósito de migración. El contenido debe ser luego transformado y rescatado extrayéndolo y almacenándolo de acuerdo con ese Modelo de Contenido en el Servidor de Contenido Estructurado, donde esto es expuesto a través de una API HTTP (ya sea REST o algo como el reciente GrahpQL) para ser consumido por Aplicaciones  Cliente Web y Aplicaciones Nativas. El flujo de Editorial debe ser diseñado, y un Primer API CMS implementado. Alternativas de Pilas arquitectónicas que podrían ser usadas para implementar todo esto deben ser evaluadas y las arquitecturas candidatas priorizadas, planificadas y presupuestadas, implementadas y desplegadas.
